...
`;
  ...
  const handleAIHtmlGeneration = async (prompt, options = {}) => {
    ...
    try {
      const aiService = getAIService();
      const htmlContent = await aiService.generateHTML(prompt, false); // Set to true for interactive content
  ...
  return (
    <form onSubmit={e => { e.preventDefault(); handlePreview(); }} aria-label="HTML Code Submission">
      ...
      <AIPromptInput
        ...
        showSlideCount={false}
        isConfigured={!!(window.aiConfig?.apiKey || window.aiConfig?.provider === 'ollama')}
        placeholder="Describe the HTML content you want to create (e.g., 'Interactive timeline of World War II with collapsible sections and images')"
      />
